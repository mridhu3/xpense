...
export default function SettingsScreen() {
  ...
  return <ScreenContainer edges={["top", "bottom", "left", "right"]} style={styles.screen}><ScrollView contentContainerStyle={styles.content}>
    ...
    <Text style={styles.label}>PREFERENCES</Text><Card style={styles.card}><ToggleRow icon="notifications-none" title="Smart nudges" detail="Warn me before I overspend" value={nudges} onChange={setNudges} /><ToggleRow icon="notifications-active" title="Push notifications" detail="Receive reminders on this device" value={pushEnabled} onChange={enablePush} /><ToggleRow icon="assessment" title="Weekly report card" detail="Get a Sunday money recap" value={weekly} onChange={setWeekly} /><Pressable onPress={sendTest} style={({ pressed }) => [styles.testButton, pressed && { opacity: 0.7 }]}><MaterialIcons name="send" size={15} color={colors.orange} /><Text style={styles.testText}>Send a test alert</Text></Pressable>{pushStatus ? <Text style={styles.pushStatus}>{pushStatus}</Text> : null}</Card>
    <Text style={styles.label}>YOUR DATA</Text><Card style={styles.card}><SettingRow icon="cloud-done" title="Sync & backup" detail="Local-first · Last synced just now" /><SettingRow icon="delete-outline" title="Reset demo workspace" detail="Clear local data and start fresh" danger /></Card>
    <Text style={styles.label}>CONNECT</Text><UpiPaymentCard /><Card style={styles.card}><SettingRow icon="account-balance" title="Bank / UPI sync" detail="Secure read-only provider connection" /></Card>
